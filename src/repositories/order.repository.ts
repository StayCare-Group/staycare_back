...
export interface IOrderMySQL {
  ...
  order_number: string;
  client_id: EntityId;
  property_id: EntityId | null;
  driver_id: EntityId | null;
...
export class OrderRepository {
  ...
  static async insert(conn: PoolConnection, data: Omit<IOrderMySQL, "id" | "created_at" | "updated_at">): Promise<EntityId> {
    ...
    await conn.execute(
      `INSERT INTO orders (
        id, order_number, client_id, property_id, driver_id, service_type, 
  ...
  static async deleteItemsByOrderId(conn: PoolConnection, orderId: EntityId): Promise<void> {
    await conn.execute("DELETE FROM order_items WHERE order_id = ?", [orderId]);
  }

  static async insertItem(conn: PoolConnection, item: Omit<IOrderItemMySQL, "id">): Promise<EntityId> {
    const id = generateEntityId();
    await conn.execute(
      `INSERT INTO order_items (
        id, order_id, item_id, item_code_snapshot, name_snapshot, quantity, unit_price, total_price,
  ...
  static async updateItem(conn: PoolConnection | null, id: EntityId, data: Partial<IOrderItemMySQL>): Promise<void> {
  ...
  static async insertHistory(conn: PoolConnection, history: Omit<IOrderStatusHistoryMySQL, "id" | "changed_at">): Promise<EntityId> {
    const id = generateEntityId();
    await conn.execute(
      `INSERT INTO order_status_history (
        id, order_id, changed_by_user_id, is_system, status, note
      ) VALUES (?, ?, ?, ?, ?, ?)`,
      [
        id,
  ...
  static async delete(id: EntityId, conn?: PoolConnection): Promise<void> {
    const exec = conn || pool;
  ...
  static async existsByPropertyId(propertyId: EntityId): Promise<boolean> {
    const [rows] = await pool.execute<RowDataPacket[]>(
